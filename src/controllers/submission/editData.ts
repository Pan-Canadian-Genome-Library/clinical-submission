...
export const editData = validateRequest(
  ...
  async (req, res, next) => {
    try {
      ...
      const currentDictionary =
        await lyricProvider.services.dictionary.getActiveDictionaryByCategory(
          categoryId
        );

      if (!currentDictionary) {
        throw new lyricProvider.utils.errors.BadRequest(
          `Dictionary in category '${categoryId}' not found`
        );
      }
      ...
      for (const file of files) {
        try {
          ...
          const schema = currentDictionary.dictionary.find(
          ...
          const { file: prevalidatedFile, error } = await prevalidateEditFile(
            file,
            schema
          );
          if (error) {
          ...
          const extractedData = await parseFileToRecords(
            prevalidatedFile,
            schema
          );

          const uploadResult =
            await lyricProvider.services.submittedData.editSubmittedData({
