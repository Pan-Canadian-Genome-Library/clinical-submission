...
| `RESTRICT_READ_ACCESS_ENABLED` | Master switch for restricting users who have READ-but-not-WRITE access to a study: their responses have restricted schemas and fields filtered out (see `RESTRICTED_SCHEMA_NAMES` and `RESTRICTED_FIELD_NAMES`), and they cannot query data marked as restricted.                                                                                        | true                                   |
| `RESTRICTED_SCHEMA_NAMES`   | Comma-separated list of schema/entity names to exclude entirely from responses for READ-only users. (Example: sociodemographic). Unset or empty means nothing is restricted.                                                                                                                                                                                 |                                        |
| `RESTRICTED_FIELD_NAMES`    | Comma-separated list of field names to remove from responses for READ-only users, wherever they appear across any entity. (Example: submitter_id). Unset or empty means nothing is restricted.                                                                                                                                                              |                                        |
...
| `VALKEY_HOST`               | The hostname or IP address of the Valkey server used for caching and session storage.                                                                                                                                                                                                                                                                         | 'localhost'                            |
